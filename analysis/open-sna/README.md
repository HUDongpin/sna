...
| Stability | `bootnet` case-dropping bootstrap using the same NPN → Pearson → conditional-PD → EBICglasso estimator as the pooled/NCT path, correlation threshold `0.70`, one core |
| Seed | `2026` |
...
The same internal NPN → Pearson → conditional-PD → EBICglasso helper is used by the pooled network, every original/permuted NCT sample, and `bootnet` case-dropping replicates. `qgraph::EBICglasso` on an unconditioned NPN Pearson matrix stops with `'S' is not positive definite`, so NCT and bootstrap never run. That is the published-sample worker failure. Conditioning first lets EBICglasso run. On cyclic duplicate columns it can select the densest lambda and print that the sparsity assumption might be violated. `bootnet::estimateNetwork(..., default = "EBICglasso", corMethod = "npn")` is not used. That default path also stops on a non-positive-definite NPN matrix. Stability therefore calls `bootnet::estimateNetwork` with `default = "none"` and `fun = nct_npn_ebicglasso_estimator`. NCT-local and bootstrap estimator warnings and sparse-model messages are bounded internally so resampling diagnostics do not leak into aggregate result warnings. Estimation errors remain fatal. The pooled path keeps ordinary warning capture.
...
`R_ANALYSIS_FAILED` means validation already succeeded (or never ran as a wrapped `WORKBOOK_INVALID`) and estimation/runtime threw. The public sample at `public/open-sna/programming-resilience-sample.xlsx` is a schema-valid one-sheet Gender `F`/`M` Likert workbook with 16 unique construct-prefixed items; it is not the empty-network regression fixture and it is not cyclic duplicate columns. Full analysis on a non-empty graph calls `qgraph` (spring layout), `networktools::bridge`, NCT, and `bootnet`. The empty-network regression skips layout, bridge, and bootnet. Analyze mode opens `grDevices::pdf(NULL)` first so Rscript does not try to write `Rplots.pdf` on the read-only worker.

A 50-row Likert+Gender workbook that returns HTTP 500 `R_ANALYSIS_FAILED` from `127.0.0.1:3101` is therefore an engine/runtime failure. Inspect `docker logs sna-r-worker` for `open_sna_r_failed`. Do not treat that as a missing Gender column or Likert 1–5 problem; those are `422 WORKBOOK_INVALID`.
...
`app/api/open-sna/analyze/route.ts` is the analysis adapter. The default POST remains a bounded synchronous path for short runs. `delivery=async` returns HTTP 202 with a job id; `GET /api/open-sna/analyze?job=` polls that job. The adapter fast-rejects an oversized declared multipart body, validates workbook size, extension, media type, ZIP signature, fixed resampling settings, result size, and the JSON contract, and removes each synchronous per-job directory in `finally`. Local development uses a temporary root below `/Volumes/Starship/`. Dedicated worker mode uses an isolated `/tmp/open-sna-*` or `/var/tmp/open-sna-*` root, requires a Bearer token of at least 32 characters, admits one R job per process, and returns `429 WORKER_BUSY` for a second concurrent synchronous analysis. Async submissions enqueue behind that single slot instead of holding the gateway request open.

The Vercel adapter requires `OPEN_SNA_R_API_URL` and `OPEN_SNA_R_API_TOKEN` together. Non-loopback worker URLs must use HTTPS. Worker diagnostics are mapped to a small public error contract and are not relayed to the browser. A worker `500 R_ANALYSIS_FAILED` becomes `502 R_ANALYSIS_FAILED` on Vercel. The Vercel adapter copies workbook bytes into a `Blob` and `FormData.append`s them as `input.xlsx` so undici does not forward an empty multipart part. A 200 worker body that fails the Open SNA contract becomes `502 R_ENGINE_CONTRACT_FAILED` (logged as `open_sna_remote_engine_failed`); `R_ENGINE_UNAVAILABLE` is reserved for transport failures, not a parsed R engine error. On the worker, a non-zero R exit logs one redacted `open_sna_r_failed` JSON line (failure code, exit code, timeout flag, empty-stderr flag, and the path-stripped `Open SNA analysis failed:` message). Captured R stderr is a 64 KiB tail used only to parse `OPEN_SNA_ERROR_CODE`; exceeding that cap no longer kills the process. During the schema `1.1` rollout only, this remote boundary can normalize a structurally valid `1.0` worker result to `1.1`; local R output, browser responses, the bundled demo, and AI input are `1.1` only. Legacy results that omit the required two-group/NCT contract still fail closed. After the aggregate R contract is validated, the Vercel adapter can request a strict JSON-schema interpretation from the model pinned as `openai/gpt-5.6-luna`. `request.formData()` is not a streaming transport limit, and a ZIP signature is not an uncompressed-size defense. The production gateway must still enforce request-body, expanded-ZIP, entry-count, memory, CPU, and rate limits before analysis.

Vercel does not provide this repository's R runtime. `Dockerfile.open-sna-worker` therefore defines the separate worker image with R 4.4.2, a complete 159-package `renv.lock`, an R preflight, a non-root runtime user, and a process health check. The worker restores the upload-to-result loop with one active R job per process. The workbench uses the async enqueue and poll path so a 1,000-bootstrap run is not owned by the Vercel request lifetime. Durable object storage, cancellation, and Cloud Tasks remain out of this increment. The bundled Programming Resilience result remains a precomputed aggregate reference and is never a fallback for a failed upload.

## Current timing boundary

The observed local empty-network regression averages about **193 seconds per analysis**, which is close to the 255-second synchronous POST limit. The workbench therefore submits `delivery=async` and polls `GET /api/open-sna/analyze?job=`. Synchronous POST remains available for short API checks. A hung async R child is still reaped by a longer worker-only timer. Target-container timing should still be measured against the final worker configuration.
