...
import { fillOpenSna, type OpenSnaCopy } from "@/lib/open-sna-copy";
...
function nodeMetric(value: number | null, notAvailable: string) {
  return value === null ? notAvailable : value.toFixed(3);
}

export default function NetworkGraph({ result, copy }: { result: OpenSnaResult; copy: OpenSnaCopy }) {
  ...
  return (
    <figure aria-labelledby="open-sna-network-title" aria-describedby="open-sna-network-summary">
      <div className="overflow-hidden rounded-[1.6rem] border border-[var(--line)] bg-[var(--page)]">
          ...
          <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
            <div>
              <div className="flex flex-wrap items-center gap-2">
                <h3 id="open-sna-network-title" className="text-lg font-black text-[var(--ink)]">{graph.title}</h3>
                <span className="rounded-full bg-[var(--surface-soft)] px-2.5 py-1 text-xs font-black text-[var(--indigo)]" aria-live="polite">
                  {fillOpenSna(graph.edgeCount, { visible: filteredEdges.length, total: result.edges.length })}
                </span>
              </div>
            ...
            <div className="flex items-center gap-1 rounded-xl border border-[var(--line)] bg-[var(--page)] p-1" role="group" aria-label={graph.zoomControls}>
              <button type="button" onClick={() => setZoom((value) => Math.max(1, Number((value - 0.25).toFixed(2))))} disabled={zoom <= 1} className="focus-ring grid h-11 w-11 cursor-pointer place-items-center rounded-lg text-[var(--muted)] transition hover:bg-[var(--surface-soft)] hover:text-[var(--indigo)] disabled:cursor-not-allowed disabled:opacity-40" aria-label={graph.zoomOut} title={graph.zoomOut}><ZoomIcon direction="out" /></button>
              ...
              <button type="button" onClick={() => setZoom((value) => Math.min(1.75, Number((value + 0.25).toFixed(2))))} disabled={zoom >= 1.75} className="focus-ring grid h-11 w-11 cursor-pointer place-items-center rounded-lg text-[var(--muted)] transition hover:bg-[var(--surface-soft)] hover:text-[var(--indigo)] disabled:cursor-not-allowed disabled:opacity-40" aria-label={graph.zoomIn} title={graph.zoomIn}><ZoomIcon direction="in" /></button>
              <button type="button" onClick={() => { setZoom(1); setSelectedNodeId(null); }} className="focus-ring grid h-11 w-11 cursor-pointer place-items-center rounded-lg text-[var(--muted)] transition hover:bg-[var(--surface-soft)] hover:text-[var(--indigo)]" aria-label={graph.resetNetwork} title={graph.resetView}><ZoomIcon direction="reset" /></button>
            </div>
          </div>

          <div className="mt-4 grid gap-4 lg:grid-cols-[minmax(0,1fr)_minmax(14rem,0.65fr)] lg:items-end">
            <fieldset>
              <legend className="text-xs font-black uppercase tracking-[0.13em] text-[var(--muted)]">{graph.visibleCommunities}</legend>
                ...
                {communities.map((community) => {
                  ...
                  return (
                    <button key={community} type="button" aria-pressed={active} onClick={() => toggleCommunity(community)} className={cn("focus-ring inline-flex min-h-11 cursor-pointer items-center gap-2 rounded-full border px-3 text-sm font-black transition", active ? "border-[var(--indigo)] bg-[var(--surface-soft)] text-[var(--ink)] shadow-sm" : "border-[var(--line)] bg-[var(--page)] text-[var(--muted)] opacity-65 hover:opacity-100")}>
                      ...
                      {community}
                      <span className="sr-only">{active ? graph.visible : graph.hidden}</span>
                    </button>
                  );
                })}
              </div>
            ...
            <label className="block text-xs font-black uppercase tracking-[0.13em] text-[var(--muted)]">
              {graph.minimumWeight} <span className="float-right text-[var(--ink)]">{minimumWeight.toFixed(2)}</span>
        ...
        <div className="grid lg:grid-cols-[minmax(0,1fr)_15rem]">
          <div className="relative min-h-[20rem] overflow-hidden bg-[var(--surface)] p-2 sm:p-4">
            <p className="sr-only" id="open-sna-network-instructions">{graph.instructions}</p>
            <svg className="h-auto w-full" viewBox={`0 0 ${width} ${height}`} role="group" aria-labelledby="open-sna-network-title open-sna-network-summary open-sna-network-instructions">
              <desc id="open-sna-network-summary">
                {fillOpenSna(graph.summary, { nodes: result.overview.nodeCount, edges: filteredEdges.length })}
              </desc>
              <g transform={`translate(${zoomAnchorX} ${zoomAnchorY}) scale(${zoom}) translate(${-zoomAnchorX} ${-zoomAnchorY})`} className="open-sna-graph-viewport">
                {filteredEdges.map((edge) => {
                  ...
                  return (
                    <line key={`${edge.source}-${edge.target}`} x1={source.x * width} y1={source.y * height} x2={target.x * width} y2={target.y * height} stroke={edge.sign === "positive" ? "var(--teal)" : "var(--danger)"} strokeWidth={Math.max(1.4, edge.absoluteWeight * 14)} strokeOpacity={connectedToActive ? (edge.relationship === "within-community" ? 0.62 : 0.88) : 0.09} strokeDasharray={edge.sign === "negative" ? "7 5" : undefined} strokeLinecap="round" className="open-sna-graph-edge">
                      <title>{fillOpenSna(graph.edgeTitle, { source: edge.source, target: edge.target, weight: edge.weight.toFixed(3) })}</title>
                    </line>
                  );
                })}
                {result.nodes.map((node) => {
                  ...
                  return (
                    <g key={node.id} transform={`translate(${node.x * width} ${node.y * height})`} role="button" tabIndex={communityActive ? 0 : -1} aria-hidden={!communityActive} aria-label={fillOpenSna(graph.nodeAria, { label: node.label, community: node.community, strength: nodeMetric(node.strength, copy.common.notAvailable), predictability: nodeMetric(node.predictability, copy.common.notAvailable), selected: selected ? graph.selectedSuffix : "" })} aria-pressed={selected} onClick={() => selectNode(node)} onKeyDown={(event) => handleNodeKeyboard(event, node)} onMouseEnter={() => communityActive && setHoveredNodeId(node.id)} onMouseLeave={() => setHoveredNodeId(null)} onFocus={() => communityActive && setHoveredNodeId(node.id)} onBlur={() => setHoveredNodeId(null)} className={cn("open-sna-graph-node", communityActive ? "cursor-pointer" : "pointer-events-none")} opacity={communityActive ? (related ? 1 : 0.28) : 0.1}>
          ...
          <aside className="border-t border-[var(--line)] bg-[var(--page)] p-5 lg:border-l lg:border-t-0" aria-label={graph.inspector} aria-live="polite">
            <p className="text-xs font-black uppercase tracking-[0.14em] text-[var(--indigo)]">{graph.inspector}</p>
            {activeNode ? (
              <div className="mt-4">
                ...
                <p className="mt-1 text-sm text-[var(--muted)]">{fillOpenSna(graph.communityLabel, { community: activeNode.community })}</p>
                <dl className="mt-5 space-y-3 text-sm">
                  <div className="flex justify-between gap-4"><dt className="text-[var(--muted)]">{graph.strength}</dt><dd className="font-black tabular-nums">{nodeMetric(activeNode.strength, copy.common.notAvailable)}</dd></div>
                  <div className="flex justify-between gap-4"><dt className="text-[var(--muted)]">{graph.predictability}</dt><dd className="font-black tabular-nums">{nodeMetric(activeNode.predictability, copy.common.notAvailable)}</dd></div>
                  <div className="flex justify-between gap-4"><dt className="text-[var(--muted)]">{graph.visibleTies}</dt><dd className="font-black tabular-nums">{filteredEdges.filter((edge) => edge.source === activeNode.id || edge.target === activeNode.id).length}</dd></div>
                </dl>
                <p className="mt-5 rounded-xl bg-[var(--surface)] p-3 text-xs leading-5 text-[var(--muted)]">
                  {selectedNodeId === activeNode.id ? graph.selectedHint : graph.previewHint}
                </p>
              </div>
            ) : (
              <div className="mt-4 rounded-xl border border-dashed border-[var(--line)] bg-[var(--surface)] p-4">
                <p className="font-black text-[var(--ink)]">{graph.selectNode}</p>
                <p className="mt-2 text-sm leading-6 text-[var(--muted)]">{graph.selectNodeHelp}</p>
              </div>
            )}
            {selectedNode && selectedConnections.length ? (
              <div className="mt-5 border-t border-[var(--line)] pt-4">
                <p className="text-xs font-black uppercase tracking-[0.12em] text-[var(--muted)]">{graph.strongestTies}</p>
      ...
      <figcaption className="mt-4 grid gap-3 text-sm leading-6 text-[var(--muted)] sm:grid-cols-[1fr_auto] sm:items-start">
        <span>{strongest ? fillOpenSna(graph.strongestEdge, { source: strongest.source, target: strongest.target, weight: strongest.weight.toFixed(3) }) : graph.noEdge} {graph.signLegend}</span>
        <span className="font-bold text-[var(--ink)]">{graph.nodeSizeLegend}</span>
      </figcaption>
    </figure>
  );
}
