...
export default function OpenSnaWorkbench({ copy, locale, htmlLang }: { copy: OpenSnaCopy; locale: Locale; htmlLang: string }) {
  ...
  async function analyzeWorkbook() {
    ...
    try {
      ...
      const decoded = await runOpenSnaWorkbookAnalysis(formData, { locale });
      if (!decoded.ok) {
      ...
      <aside id="open-sna-setup" className="surface-card scroll-mt-24 h-fit overflow-hidden xl:sticky xl:top-24">
        ...
        <aside className="m-4 mb-0 rounded-xl border border-[var(--teal-line)] bg-[var(--teal-tint)] p-4 sm:m-5 sm:mb-0" aria-label={copy.beta.ariaLabel}>
          ...
          <ul className="mt-2 list-disc space-y-1 pl-4 text-xs leading-5 text-[var(--ink)]">
            {copy.beta.items.map((item) => <li key={item}>{item}</li>)}
          </ul>
        </aside>
        ...
        <div id="open-sna-setup-controls" className={cn("space-y-5 p-4 sm:p-5", setupOpen ? "block" : "hidden", "xl:block")}>
          <div>
            <div className="flex items-center justify-between gap-3">
              <p id="open-sna-workbook-label" className="text-sm font-black text-[var(--ink)]">{copy.setup.chooseWorkbook}</p>
              <span className="text-xs font-bold text-[var(--muted)]">{copy.setup.fileLimit}</span>
            </div>
            <input ref={fileInputRef} id="open-sna-workbook" type="file" accept=".xlsx" onChange={handleWorkbookChange} className="sr-only" aria-label={copy.setup.chooseFileAria} aria-describedby="open-sna-workbook-help" />
