...
const en: OpenSnaCopy = {
  ...
  beta: {
    ...
    items: [
      ...
      "Large workbooks or analyses with 1,000 bootstrap replicates are queued and polled so the browser does not hold one request open.",
...
const zhHant: OpenSnaCopy = {
  ...
  beta: {
    ...
    items: [
      ...
      "大型活頁簿或使用 1,000 次自助法重複的分析會排隊並輪詢，瀏覽器不會長時間佔用單一請求。",
      "上傳的活頁簿與列層資料不會保留。",
      "此公開測試版不提供高可用性或可用性承諾。",
    ],
  },
...
const zhHans: OpenSnaCopy = {
  ...
  beta: {
    ...
    items: [
      ...
      "大型工作簿或使用 1,000 次自助法重复的分析会排队并轮询，浏览器不会长时间占用单个请求。",
      "上传的工作簿与行级数据不会保留。",
      "此公开测试版不提供高可用性或可用性承诺。",
    ],
  },
